...
const int BUFFER_SIZE = 2048; // Tingkatkan buffer jika pesan JSON besar
...
inline std::string getCurrentTimestamp() {
    ...
    // Format ke ISO 8601. std::put_time tidak sepenuhnya mendukung milidetik secara portabel.
    // Ini akan menghasilkan YYYY-MM-DD HH:MM:SS, kita bisa tambahkan 'T' dan 'Z' jika perlu presisi lebih.
    // Atau gunakan pustaka date/time yang lebih canggih jika format presisi tinggi dibutuhkan secara konsisten.
    ss << std::put_time(std::localtime(&itt), "%Y-%m-%dT%H:%M:%S");
    // Tambahkan milidetik secara manual (kurang ideal tapi cukup untuk contoh)
